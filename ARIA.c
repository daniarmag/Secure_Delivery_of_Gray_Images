...
// Diffusion Layer
void DiffusionLayer (const Byte *i, Byte *o)
{
	Byte T;
...
// w0 : master key, e : encryption round keys, keyBits : number of bits in key
// return: R : number of rounds
int EncKeySetup(const Byte *w0, Byte *e, int keyBits) {
	int  i, R=(keyBits+256)/32, q;
	Byte t[16], w1[16], w2[16], w3[16];
  
	q = (keyBits - 128) / 64;
	for (i = 0; i < 16; i++) t[i] = SBox[     i  % 4][CK[q][i] ^ w0[i]];
	DiffusionLayer (t, w1);
	if (R==14)
	...
	q = (q==2)? 0 : (q+1);
	for (i = 0; i < 16; i++) t[i] = SBox[(2 + i) % 4][CK[q][i] ^ w1[i]];
	DiffusionLayer (t, w2);
	for (i = 0; i < 16; i++) w2[i] ^= w0[i];
  
	q = (q==2)? 0 : (q+1);
	for (i = 0; i < 16; i++) t[i] = SBox[     i  % 4][CK[q][i] ^ w2[i]];
	DiffusionLayer (t, w3);
...
// w0 : maskter key, d : decryption round keys, keyBits : number of bits in key
// return: R : number of rounds
...
// p: plain text,R: number of rounds, e: round keys, c: ciphertext
